...
exports.getPosts = async (req, res) => {
  try {
    const { page, page_size, type, category_name, cateChild_name } = await Joi.object()
      .keys({
  ...
  try {
    const value = await Joi.object()
      .keys({
        ...
        slug: Joi.string().min(6).required(),
      })
...
exports.editPost = async (req, res) => {
  ...
  try {
    ...
    value.tags = await checkTag(value.tags)
